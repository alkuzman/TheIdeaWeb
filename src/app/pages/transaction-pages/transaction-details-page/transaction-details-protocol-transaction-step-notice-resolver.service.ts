...
export class TransactionDetailsProtocolTransactionStepNoticeResolverService implements Resolve<ProtocolTransactionStepNotice<any>> {
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot):
    Observable<ProtocolTransactionStepNotice<any>> | Promise<ProtocolTransactionStepNotice<any>> | ProtocolTransactionStepNotice<any> {
    const noticeId: number = +route.params['id'];
    return this.noticeService.getNotice(noticeId).pipe(
      map(n => <ProtocolTransactionStepNotice<any>>n),
      catchError((error: any) => this.errorHandlingService.handleError(error)));
  }
}
